...
服务器自用 mod，用于**有选择地加速矿车**，同时避免破坏依赖标准矿车机制的红石设备（如全物品分类器）。

具体来说，此模组为特定的矿车开启了**实验性功能**，使其可以拥有独立的最大速度。使用方法：在铁路旁放置一个告示牌（任意种类，包括悬挂告示牌），并写上以下文本：
```
*NewSpeed*
30
```
任何经过告示牌旁边的**载客矿车**（载有乘客或生物）将被升级，使用实验性物理机制，并以 30 格/秒为最大速度。你可以在铁路沿线放置多个告示牌，为不同路段设置不同的限速。空矿车及其他类型的矿车完全不受影响。
